...
export function renderCollaborationLayout() {
    ...
    return `
        <div class="flex-1 w-full flex overflow-hidden min-w-0">
...
function renderProjectSidebar(projects, myInvites) {
    ...
    return `
        <div class="w-48 md:w-56 shrink-0 border-r border-[#E2E8F0] bg-white p-4 overflow-y-auto jara-scroll">
            <div class="flex items-center justify-between mb-3">
                <h3 class="font-display font-bold text-xs text-[#94A3B8] uppercase tracking-wide">Proyek</h3>
                ...
                : projects.map((p) => `
                    <button data-action="select-project" data-project="${p.id}"
                        ...
                        <div class="min-w-0">
                            ...
                            <p class="text-xs text-[#94A3B8] font-normal">${p.members.length} anggota</p>
                        </div>
                    </button>
                `).join('')}
            ...
            ${myInvites.length > 0 ? `
                <div class="mt-4">
                    <h3 class="font-display font-bold text-xs text-[#94A3B8] uppercase tracking-wide mb-2">Undangan Saya</h3>
                    ${myInvites.map((inv) => `
                        <div class="p-3 rounded-xl bg-[#FEF9C3] border border-yellow-200 mb-2">
                            ...
                            <p class="text-xs text-[#78350F] mt-0.5">dari ${inv.invitedBy ? escapeHtml(inv.invitedBy.name) : 'Pemilik'}</p>
                            <div class="flex gap-1.5 mt-2">
                                <button data-action="respond-invite" data-invite="${inv.id}" data-accept="1"
                                    class="flex-1 text-xs py-1 rounded-lg bg-[#10B981] text-white font-medium hover:bg-emerald-600">Terima</button>
                                <button data-action="respond-invite" data-invite="${inv.id}" data-accept="0"
                                    class="flex-1 text-xs py-1 rounded-lg bg-white border border-[#E2E8F0] text-[#64748B] hover:bg-[#F8FAFC]">Tolak</button>
                            </div>
                        </div>
                    `).join('')}
                </div>
    ...
    if (project.members.length === 0) {
        return `
            <div class="text-center py-12">
                ...
                <p class="font-display font-bold text-[#1E293B]">Belum ada anggota</p>
                ...
                return `
                    <div class="bg-white rounded-xl border border-[#E2E8F0] p-4 flex items-center gap-4">
                        ...
                        <div class="flex-1 min-w-0">
                            <div class="flex items-center gap-2">
                                <p class="font-semibold text-sm text-[#1E293B] truncate">${escapeHtml(memberUser.name)} ${isSelf ? '<span class="text-xs text-[#94A3B8]">(kamu)</span>' : ''}</p>
                                ${roleBadge(m.role)}
                            </div>
                            <p class="text-xs text-[#94A3B8] mt-0.5">${escapeHtml(memberUser.email)}</p>
                            <div class="flex items-center gap-3 mt-2">
                                <span class="text-xs text-[#64748B]">${doneCount}/${memberTasks.length} tugas selesai</span>
                        ...
                        <div class="flex items-center gap-2 shrink-0">
                            ...
                            <span class="text-xs text-[#94A3B8] hidden sm:inline">${memberUser.status === 'active' ? 'Aktif' : 'Nonaktif'}</span>
                            ${owner && m.role !== 'owner' ? `
                                <button data-action="open-remove" data-user="${m.userId}" title="Hapus anggota"
    ...
    if (invitations.length === 0) {
        return `
            <div class="text-center py-12">
                ...
                <p class="font-display font-bold text-[#1E293B]">Belum ada undangan</p>
                ...
                return `
                    <div class="bg-white rounded-xl border border-[#E2E8F0] p-4 flex items-center gap-4">
                        ...
                        <div class="flex-1 min-w-0">
                            ...
                            <p class="text-xs text-[#94A3B8] mt-0.5">${escapeHtml(email)} · Dikirim ${escapeHtml(inv.sentAt)}</p>
                        </div>
                        ${inviteBadge(inv.status)}
                        ${owner && inv.status === 'pending' ? `
                            <button data-action="cancel-invite" data-invite="${inv.id}"
                                class="p-1.5 rounded-lg hover:bg-red-50 text-[#94A3B8] hover:text-red-500" title="Batalkan undangan">
...
function renderTasksTab(project) {
    ...
    return `
        <div class="flex items-center justify-between mb-4">
            <p class="text-xs text-[#94A3B8]">${projectTasks.length} tugas · ${projectTasks.filter((t) => t.status === 'completed').length} selesai</p>
            ${owner ? `
        ...
        ${projectTasks.length === 0 ? `
            <div class="text-center py-12">
                <div class="w-14 h-14 rounded-2xl bg-[#E8F9F9] flex items-center justify-center mx-auto mb-3">
                    <svg viewBox="0 0 24 24" fill="none" stroke="#0BC5C1" stroke-width="1.5" class="w-7 h-7"><path d="M9 11l3 3L22 4"/><path d="M21 12v7a2 2 0 01-2 2H5a2 2 0 01-2-2V5a2 2 0 012-2h11"/></svg>
                </div>
                <p class="font-display font-bold text-[#1E293B]">Belum ada tugas</p>
                <p class="text-sm text-[#94A3B8] mt-1">Belum ada tugas di proyek ini.</p>
            </div>
        ` : `
            <div class="space-y-2">
                ${projectTasks.map((task) => {
                    const assignee = getUser(task.assigneeId);
                    // FR-23: hanya assignee yang boleh ubah status.
                    // Owner read-only untuk task anggota (monitoring saja),
                    // member read-only untuk task yang bukan miliknya.
                    const canChangeStatus = task.assigneeId !== null && task.assigneeId === user.id;

                    return `
                        <div class="bg-white rounded-xl border border-[#E2E8F0] p-4 space-y-3">
                            <div class="flex items-start gap-3 min-w-0">
                                <div class="w-2 h-2 rounded-full shrink-0 mt-1.5" style="background:${statusColor(task.status)}"></div>
                                <div class="flex-1 min-w-0">
                                    <p class="text-sm font-medium text-[#1E293B] break-words">${escapeHtml(task.title)}</p>
                                    <div class="flex items-center gap-2 mt-1.5 flex-wrap">
                                        ${statusBadge(task.status)}
                                        ${priorityBadge(task.priority)}
                                        <span class="text-xs text-[#94A3B8]">Tenggat ${escapeHtml(task.deadline || '—')}</span>
                                    </div>
                                </div>
                            </div>

                            <div class="flex items-center gap-2 flex-wrap pl-5 pt-3 border-t border-[#F1F5F9]">
                                ${task.assigneeId && assignee ? `
                                    <div class="flex items-center gap-2 min-w-0">
                                        ${avatar(assignee, 'w-7 h-7 text-xs')}
                                        <span class="text-xs text-[#64748B] truncate">${escapeHtml(assignee.name)}</span>
                                    </div>
                                ` : (owner ? '' : `<span class="text-xs text-[#94A3B8] px-2 py-1 bg-[#F8FAFC] rounded-lg shrink-0">Belum ditugaskan</span>`)}

                                ${owner ? `
                                    <label class="flex items-center gap-1.5 min-w-0">
                                        <span class="text-xs text-[#94A3B8] shrink-0">Tugaskan:</span>
                                        <select data-action="assign-task" data-task="${task.id}"
                                            class="min-w-0 max-w-full px-2 py-1.5 rounded-lg border border-[#E2E8F0] bg-white text-xs text-[#64748B] outline-none focus:border-[#0BC5C1]">
                                ...
                                ${canChangeStatus ? `
                                    <label class="flex items-center gap-1.5 shrink-0 ml-auto">
                                        <span class="text-xs text-[#94A3B8] shrink-0">Status:</span>
    ...
    return `
        <div class="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4" data-action="close-invite">
            <div class="bg-white rounded-2xl w-full max-w-md shadow-2xl p-6" data-stop>
                <h2 class="font-display font-bold text-lg text-[#1E293B] mb-1">Undang Anggota</h2>
                <p class="text-sm text-[#94A3B8] mb-5">Undang pengguna untuk berkolaborasi di <strong class="text-[#1E293B]">${escapeHtml(project.name)}</strong></p>
                ...
                ${candidates.length === 0 ? `
                    <div class="p-4 rounded-xl bg-[#F8FAFC] border border-[#E2E8F0] text-center">
                        <p class="text-sm text-[#64748B]">Tidak ada pengguna yang dapat diundang.</p>
                        <p class="text-xs text-[#94A3B8] mt-1">Semua pengguna aktif sudah menjadi anggota atau memiliki undangan tertunda.</p>
                    </div>
                ` : `
                    <div class="mb-4">
                        <p class="text-xs font-semibold text-[#475569] uppercase tracking-wide mb-2">Pilih pengguna</p>
                ...
                <div class="flex gap-3">
                    <button data-action="close-invite" class="flex-1 py-2.5 rounded-xl border border-[#E2E8F0] text-sm text-[#64748B] hover:bg-[#F8FAFC]">Batal</button>
                    <button data-action="send-invite" ${state.inviteSuccess ? 'disabled' : ''}
                        class="flex-1 py-2.5 rounded-xl bg-[#0BC5C1] text-white text-sm font-semibold hover:bg-[#0AAEAA] disabled:opacity-50 disabled:cursor-not-allowed">Kirim Undangan</button>
                </div>
            </div>
        </div>
    `;
    ...
    return `
        <div class="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4" data-action="close-new-task">
            <div class="bg-white rounded-2xl w-full max-w-sm shadow-2xl p-6" data-stop>
                <h2 class="font-display font-bold text-lg text-[#1E293B] mb-4">Tugas Baru</h2>
                <form data-form="new-task" class="space-y-4">
                    <div>
                        <label class="text-xs font-semibold text-[#475569] uppercase tracking-wide block mb-1.5">Judul Tugas *</label>
                        <input name="title" type="text" placeholder="Judul tugas..." autocomplete="off"
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        <div>
                            <label class="text-xs font-semibold text-[#475569] uppercase tracking-wide block mb-1.5">Prioritas</label>
                        ...
                        <div>
                            <label class="text-xs font-semibold text-[#475569] uppercase tracking-wide block mb-1.5">Tenggat Waktu</label>
                    ...
                    <div class="flex gap-3 pt-1">
                        <button type="button" data-action="close-new-task" class="flex-1 py-2.5 rounded-xl border border-[#E2E8F0] text-sm text-[#64748B] hover:bg-[#F8FAFC]">Batal</button>
                        <button type="submit" class="flex-1 py-2.5 rounded-xl bg-[#0BC5C1] text-white text-sm font-semibold hover:bg-[#0AAEAA]">Buat Tugas</button>
                    </div>
                </form>
            </div>
        </div>
    ...
    return `
        <div class="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4" data-action="close-remove">
            <div class="bg-white rounded-2xl p-6 w-full max-w-sm shadow-2xl" data-stop>
                ...
                <h3 class="font-display font-bold text-lg text-[#1E293B] text-center mb-1">Hapus Anggota</h3>
                <p class="text-sm text-[#64748B] text-center mb-6">
                    Hapus <strong class="text-[#1E293B]">${escapeHtml(target.name)}</strong> dari proyek <strong class="text-[#1E293B]">${escapeHtml(project.name)}</strong>? Tugas yang ditugaskan kepadanya akan menjadi belum ditugaskan.
                </p>
                <div class="flex gap-3">
                    <button data-action="close-remove" class="flex-1 py-2.5 rounded-xl border border-[#E2E8F0] text-sm text-[#64748B] hover:bg-[#F8FAFC]">Batal</button>
                    <button data-action="confirm-remove" class="flex-1 py-2.5 rounded-xl bg-red-500 text-white text-sm font-semibold hover:bg-red-600">Hapus</button>
                </div>
            </div>
        </div>
    `;
    ...
    return `
        <div class="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4" data-action="close-delete-project">
            <div class="bg-white rounded-2xl p-6 w-full max-w-sm shadow-2xl" data-stop>
                ...
                <p class="text-sm text-[#64748B] text-center mb-6">
                    Hapus proyek <strong class="text-[#1E293B]">${escapeHtml(project.name)}</strong> beserta ${taskCount} tugas di dalamnya? Tindakan ini tidak dapat dibatalkan.
                </p>
                <div class="flex gap-3">
                    <button data-action="close-delete-project" class="flex-1 py-2.5 rounded-xl border border-[#E2E8F0] text-sm text-[#64748B] hover:bg-[#F8FAFC]">Batal</button>
                    <button data-action="confirm-delete-project" class="flex-1 py-2.5 rounded-xl bg-red-500 text-white text-sm font-semibold hover:bg-red-600">Hapus</button>
                </div>
            </div>
        </div>
    `;
...
function renderNewProjectModal() {
    const state = getState();
    const draft = state.newProjectDraft ?? { name: '', description: '', deadline: '' };
    return `
        <div class="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4" data-action="close-new-project">
            <div class="bg-white rounded-2xl w-full max-w-md shadow-2xl" data-stop>
                <div class="p-6 border-b border-[#E2E8F0] flex items-center justify-between">
                    <h2 class="font-display font-bold text-lg text-[#1E293B]">Proyek Baru</h2>
                ...
                <form data-form="new-project" class="p-6 space-y-4">
                    <div>
                        <label class="text-xs font-semibold text-[#475569] uppercase tracking-wide block mb-1.5">Nama Proyek *</label>
                        <input name="name" type="text" placeholder="Nama proyek..." autocomplete="off" value="${escapeHtml(draft.name)}"
                            class="w-full px-4 py-2.5 rounded-xl border border-[#E2E8F0] text-sm text-[#1E293B] outline-none focus:ring-2 focus:ring-[#0BC5C1]/30 focus:border-[#0BC5C1]" />
                    </div>
                    <div>
                        <label class="text-xs font-semibold text-[#475569] uppercase tracking-wide block mb-1.5">Deskripsi</label>
                        <textarea name="description" rows="2" placeholder="Proyek ini tentang apa?"
                            class="w-full px-4 py-2.5 rounded-xl border border-[#E2E8F0] text-sm text-[#1E293B] outline-none focus:ring-2 focus:ring-[#0BC5C1]/30 focus:border-[#0BC5C1] resize-none">${escapeHtml(draft.description)}</textarea>
                    </div>
                    <div>
                        <label class="text-xs font-semibold text-[#475569] uppercase tracking-wide block mb-1.5">Warna</label>
                    ...
                    <div class="flex gap-3 justify-end pt-1">
                        <button type="button" data-action="close-new-project" class="px-5 py-2.5 rounded-xl border border-[#E2E8F0] text-sm text-[#64748B] hover:bg-[#F8FAFC]">Batal</button>
                        <button type="submit" class="px-5 py-2.5 rounded-xl bg-[#0BC5C1] text-white text-sm font-semibold hover:bg-[#0AAEAA]">Buat Proyek</button>
                    </div>
                </form>
            </div>
        </div>
